...
def train(CFR, sess, train_first, train_second, D, I_valid, D_test, logfile, i_exp):
    ...
    ''' 测试集dict '''
    dict_test = [{CFR.x: D_test[dt_i]['x'], CFR.t: D_test[dt_i]['t'], CFR.y_: D_test[dt_i]['yf'], \
        ...
        if (FLAGS.pred_output_delay > 0 and i % FLAGS.pred_output_delay == 0) or i==FLAGS.iterations-1:
            ...
            if D_test is not None:
                for dt_i in range(len(D_test)):
                    y_pred_f_test = sess.run(CFR.output, feed_dict={CFR.I: Im, CFR.x: D_test[dt_i]['x'], \
                                            CFR.t: D_test[dt_i]['t'], CFR.do_in: 1.0, CFR.do_out: 1.0})
                    y_pred_cf_test = sess.run(CFR.output, feed_dict={CFR.I: Im, CFR.x: D_test[dt_i]['x'], \
...
def run(outdir):
    ...
    I  = tf.compat.v1.placeholder("int32", shape=[None, ], name='I')   # weight
