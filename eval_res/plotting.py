...
CORR_CHOICE_SET_CONT = 'valid'

EARLY_STOP_SET_BIN = 'valid'
EARLY_STOP_CRITERION_BIN = 'policy_risk'
CONFIG_CHOICE_SET_BIN = 'valid'
CONFIG_CRITERION_BIN = 'policy_gain'
CORR_CRITERION_BIN = 'policy_gain'
CORR_CHOICE_SET_BIN = 'valid'
...
def table_str_bin(result_set, row_labels, labels_long=None, binary=False):
    if binary:
        if 'pehe' in result_set[0]:
            cols = ['pehe', 'bias_ate', 'ate_pred', 'accuracy_fact', 'auc_fact', 'f1score_fact', 'recall_fact', 'precision_fact', 'auc_cfact', 'f1score_cfact']
        else:
            cols = ['policy_risk', 'policy_gain', 'policy_risk_e', 'policy_gain_e', 'bias_att', 'accuracy_fact', 'auc_fact', 'f1score_fact', 'recall_fact', 'precision_fact']
    else:
        cols = ['pehe', 'bias_ate', 'ate_pred', 'rmse_fact', 'rmse_cfact', 'rmse_ite', 'pehe_nn']
    
    cols = [c for c in cols if c in result_set[0].keys()]
    ...
    if labels_long is not None:
        head_str = ' '*(len(labels_long[0])+3) + '| '.join(head_pad)
    else:
...
def evaluation_summary(result_set, row_labels, output_dir, labels_long=None, binary=False):
    s = ''
    for dtp in result_set[0].keys():
        s += 'Mode: %s\n' % cap(dtp)
        s += table_str_bin([results[dtp] for results in result_set], row_labels, labels_long, binary)
        s += '\n'
...
def select_parameters(results, configs, stop_set, stop_criterion, choice_set, choice_criterion):
    ...
    if stop_criterion in ['pehe', 'bias_ate', 'bias_att', 'rmse_fact', 'rmse_cfact', 'rmse_ite', 'pehe_nn', 'policy_risk', 'objective']:
        i_sel = np.argmin(results[stop_set][stop_criterion],2)
    else:
        i_sel = np.argmax(results[stop_set][stop_criterion],2)

    results_sel = {}
    for dtp in results.keys():
        results_sel[dtp] = {}
        for k in results[dtp].keys():
            # To reduce dimension
            results_sel[dtp][k] = np.sum(results[dtp][k],2)

            for ic in range(len(configs)):
                for ie in range(n_exp):
                    results_sel[dtp][k][ic,ie,] = results[dtp][k][ic,ie,i_sel[ic,ie],]
                    results_sel[dtp][k][ic,ie,] = results[dtp][k][ic,ie,i_sel[ic,ie],]
...
def plot_evaluation_data(results, configs, output_dir, filters=None, early_stop_set=None, early_stop_criterion=None, binary=False):
    ...
    if filters is not None:
        ...
        results = dict([(s,dict([(k,results[s][k][I,]) for k in results[s].keys()])) for s in results.keys()])
        configs = [configs[i] for i in I]
        print('Line 227 of poltting.py:', results['train'].keys())

    ''' Do parameter selection and early stopping '''
    if early_stop_set is None:
        if binary:
            early_stop_set = EARLY_STOP_SET_BIN
        else:
            early_stop_set = EARLY_STOP_SET_CONT

    if early_stop_criterion is None:
        if binary:
            if 'pehe' in results['train'].keys():
                early_stop_criterion = 'pehe'
            else:
                early_stop_criterion = EARLY_STOP_CRITERION_BIN
        else:
            early_stop_criterion = EARLY_STOP_CRITERION_CONT

    results_all, configs_all, labels, sort_key = select_parameters(results,
        configs, early_stop_set, early_stop_criterion, early_stop_set, early_stop_criterion)
    ...
    with open('%s/configs_sorted%s_%s_%s.txt' % (output_dir,filter_str,early_stop_criterion, early_stop_set), 'w') as f:
        f.write('\n'.join(labels_long))

    ''' Compute evaluation summary and store'''
    eval_str = evaluation_summary(results_all, labels, output_dir, labels_long = labels_long, binary=binary)

    with open('%s/results_summary_%s_%s%s.txt' % (output_dir, early_stop_set, early_stop_criterion,filter_str), 'w') as f:
...
def plot_evaluation_cont(results, configs, output_dir, data_train_path, data_test_path, filters=None,
                        early_stop_set=None, early_stop_criterion=None):
    ...
    results_all, configs_all, labels, sort_key = select_parameters(results,
        configs, early_stop_set, early_stop_criterion,
